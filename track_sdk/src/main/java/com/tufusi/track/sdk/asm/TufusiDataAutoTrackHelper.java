 ...
 * @description
 */
public class TufusiDataAutoTrackHelper {
    ...
    public static void trackViewOnClick(DialogInterface dialogInterface, int whichButton) {
        try {
            ...
            TufusiDataApi.getInstance().track("$AppClick", properties);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void trackExpandableListViewChildOnClick(ExpandableListView expandableListView, View view,
        ...
        try {
            ...
            String idString = TufusiDataPrivate.getViewId(expandableListView);
            if (!TextUtils.isEmpty(idString)) {
                properties.put("$element_id", idString);
            }
            ...
            if (view instanceof ViewGroup) {
                try {
                    ...
                    viewText = TufusiDataPrivate.traverseViewContent(stringBuilder, (ViewGroup) view);
                    if (!TextUtils.isEmpty(viewText)) {
    ...
    public static void trackViewOnClick(android.widget.AdapterView adapterView, android.view.View view, int position) {
        try {
            ...
            Activity activity = TufusiDataPrivate.getActivityFromContext(context);
            String idString = TufusiDataPrivate.getViewId(adapterView);
            if (!TextUtils.isEmpty(idString)) {
                properties.put("$element_id", idString);
            }
            ...
            if (adapterView instanceof Spinner) {
                ...
                if (view instanceof ViewGroup) {
                    ...
                    viewText = TufusiDataPrivate.getElementContent(view);
                }
                //$element_content
                if (!TextUtils.isEmpty(viewText)) {
                    properties.put("$element_content", viewText);
    ...
    public static void trackViewOnClick(View view) {
        try {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("$element_type", TufusiDataPrivate.getElementType(view));
            jsonObject.put("$element_id", TufusiDataPrivate.getViewId(view));
            jsonObject.put("$element_content", TufusiDataPrivate.getElementContent(view));

            Activity activity = TufusiDataPrivate.getActivityFromView(view);
            if (activity != null) {
                jsonObject.put("$activity", activity.getClass().getCanonicalName());
            }
